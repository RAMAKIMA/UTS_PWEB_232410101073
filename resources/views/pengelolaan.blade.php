    ...
    <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
      ...
      <tbody class="text-gray-800">
        ...
        <tr class="border-b hover:bg-gray-50">
          ...
          <td class="px-6 py-4 text-right">Rp {{ number_format($item['investasi_awal'], 2, ',', '.') }}</td>
          <td class="px-6 py-4 text-right">
            <span class="font-semibold {{ $item['return'] >= 0 ? 'text-green-500' : 'text-red-500' }}">
              {{ number_format($item['return'], 2, ',', '.') }} %
            </span>
          </td>
          <td class="px-6 py-4 text-right">Rp {{ number_format($item['nilai_sekarang'], 2, ',', '.') }}</td>

...
<div class="h-[32em] mt-16 mb-7 max-w-2xl mx-auto bg-white/80 backdrop-blur-md p-8 rounded-2xl shadow-xl">
    ...
    <div class="mt-10 flex items-center space-x-6 mb-6">
      ...
      <div>
        <h2 class="text-2xl font-semibold text-gray-800">{{ $profil['nama_lengkap'] }}</h2>
        <p class="text-gray-600 text-sm">{{ $profil['email'] }}</p>
        <p class="text-gray-500 text-sm italic">Username: {{ $username }}</p>
      </div>
    </div>
    ...
    <div class="mt-14 grid grid-cols-1 gap-6 text-gray-700">
      <div>
        ...
        <p class="text-base">{{ $profil['profil_risiko'] }}</p>
      </div>
      <div>
        <label class="font-semibold block text-gray-800">Tujuan Investasi:</label>
        <p class="text-base">{{ $profil['tujuan_investasi'] }}</p>
      </div>
    </div>

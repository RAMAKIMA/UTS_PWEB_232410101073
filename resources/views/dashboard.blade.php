...
<div class="p-6 bg-gray-50 min-h-screen">
  <h1 class="text-2xl font-semibold mb-6">Selamat datang, {{ $username }}!</h1>

  <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
    <!-- Total Investasi -->
    <div class="bg-white p-4 rounded shadow border-l-4 border-blue-500">
      <p class="text-gray-700">Total Investasi</p>
      <p class="text-xl font-bold text-blue-500">Rp {{ number_format($totalInvestasi, 0, ',', '.') }}</p>
    </div>

    <!-- Return -->
    <div class="bg-white p-4 rounded shadow border-l-4 border-green-500">
      <p class="text-gray-700">Return</p>
      <p class="text-xl font-bold text-green-500">{{ $return }}%</p>
    </div>

    <!-- Risiko -->
    <div class="bg-white p-4 rounded shadow border-l-4 border-orange-500">
      <p class="text-gray-700">Profil Risiko</p>
      <p class="text-xl font-bold text-orange-500">{{ $profil['profil_risiko'] ?? 'Moderate' }}</p>
    </div>

    <!-- Tujuan -->
    <div class="bg-white p-4 rounded shadow border-l-4 border-purple-500">
      <p class="text-gray-700">Tujuan Investasi</p>
      <p class="text-sm text-gray-600">{{ $profil['tujuan_investasi'] ?? ' Tabungan Jangka Panjang' }}</p>
    </div>
  </div>
...
<div class="bg-white p-6 rounded shadow mb-6">
    ...
    <div class="space-y-4">
      @foreach ($komposisi as $tipe => $persen)
        @php
          $warna = match($tipe) {
            'Saham' => 'bg-red-500',
        ...
        <div class="p-4 bg-gray-100 rounded-lg shadow-sm">
          <div class="flex justify-between items-center mb-2">
            ...
            <span class="text-sm text-gray-600">{{ $persen }}%</span>
          </div>
          <div class="w-full bg-gray-300 h-3 rounded-full overflow-hidden">
            <div class="h-3 rounded-full transition-all duration-500 ease-out {{ $warna }}"
